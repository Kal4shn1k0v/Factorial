...
public class ContainerBasicFurnace extends Container{
	private final TileEntityBasicFurnace tileentity;
	private int cookTime, totalCookTime, burnTime, currentBurnTime;
	
	public ContainerBasicFurnace(InventoryPlayer player, TileEntityBasicFurnace tileentity) 
	{
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) 
	{
		...
		if(slot != null && slot.getHasStack()) 
		{
			...
			{		
				...
				if(!BasicFurnaceRecipes.getInstance().getSinteringResult(stack1, slot1.getStack()).isEmpty())
				{
					if(!this.mergeItemStack(stack1, 0, 2, false)) 
					{
						return ItemStack.EMPTY;
					}
					else if(TileEntityBasicFurnace.isItemFuel(stack1))
					{
